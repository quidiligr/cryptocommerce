...
exports.add_product = async function(req_session,req_body){
    ...
    */
   let tmp_ids = await productsTable.find({createdby:username,status:0},{id:1})
   if(tmp_ids.length >= 5){
       
       return {statusCode:200,data:tmp_ids[0].id}
   }
   ...
   let new_id = `${username}-${created}`
        ...
        console.log(`166: OK`)
        return {statusCode:200, data: product.id}
...
exports.remove_product = async function(req_session,req_params) { 
   ...
   // console.log(`202: Product qry = ${JSON.stringify(qry,null,4)}`)
    ...
    await productsTable.remove({createdby:username, guid:req_params.guid})
    }
...
exports.edit_product = async function(req_session,req_params) { 
    ...
    */
    ...
    let qry = {createdby:username, guid:guid}
    console.log(`202: Product qry = ${JSON.stringify(qry,null,4)}`)
    ...
    if(existing_product["stock"] == null){
        qry_set["stock"] = {count:0}
        existing_product["stock"]  = qry_set.stock
    }
    if(existing_product["sales"] == null){
        qry_set["sales"] = {count:0}
        existing_product["sales"]  = qry_set.sales
    }
    if(existing_product["returns"] == null){
        qry_set["returns"] = {count:0}
        existing_product["returns"]  = qry_set.returns
    }
...
exports.save_product = async function(req) { 
    ...
    */
    ...
    let qry_product = {createdby:username, guid:req_body.guid}
...
exports.order_list = async function(req_session,req_body){
    ...
    */
    ...
    data['products'] = products
    data['current_url'] = '/vendor/order-list'
...
exports.offer_list = async function(req_session,req_body){
    ...
    */
    ...
    data['current_url'] = '/vendor/offer-list'
